...
//MessageBox.Show(string.Format("startuppath:{0}", path));
			...
			if (Debugger.IsAttached == true)
			{
				...
				for ( ; path != null; )
				{
					if (Directory.Exists(path + @"\" + tree) == true)
					{
						break;
					}
...
//MessageBox.Show(string.Format("search:{0}", path + tree));
			if (Directory.Exists(path + tree) == false)
			{
				Directory.CreateDirectory(path + tree);
			}
		...
		static RegistryKey PrepareRegKey(string field)
		{
			...
			if (reg_tree == null)
			{
				Debug.WriteLine("キーの位置が指定されていません.");
				return null;
			}
			...
			{
				Debug.WriteLine("キーのアクセスに失敗しました.");
				return null;
			}
		}
