...
{
    /// <summary>
    /// ReadJEnc ファイル読み出し＆ファイル文字コード種類自動判別(Rev.20141206)
    /// </summary>
    public class FileReader : IDisposable
    {   ////////////////////////////////////////////////////////////////////////
        // <FileReader.cs> ReadJEnc File読出＆文字コード自動判別(Rev.20141206)
        ...
        /// <param name="Length">最大読み出しファイルサイズ（領域バッファ確保サイズ）</param>
        public FileReader(int Length)
        {
        ...
        public virtual CharCode Read(FileInfo File)
        {
            ...
            {   //無用なDiskIOを極力行わないよう、オープン前にもファイルサイズチェック
                ...
                {   //BOMありテキストなら文字列を取り出す（取り出せなかったら非テキスト扱い）
                    if ((String = c.GetString(Bytes, Length)) == null) { c = null; }
                }
                else if (c == null)
                {   //ファイル文字コード種類不明なら、全バイト走査して文字コード確定
                    c = ReadJEnc.GetEncoding(Bytes, Length, out String);
                }
        ...
        public string Text { get { return String; } }
        ...
        /// <param name="Length">ファイルサイズ(未読込部分も含む。読み込み済サイズはthis.Lengthを参照)</param>
        /// <returns>確定した場合、ファイル文字コード種類。確定できなかった場合null</returns>
        protected virtual CharCode GetPreamble(long Length)
        {
            ...
            {   //UTF16Nの可能性がなければバイナリファイルとみなす
                if (ReadJEnc.SeemsUTF16N(this.Bytes, (int)Length) == null)
