...
{
    /// <summary>
    /// ReadJEnc 文字コード種類定義(Rev.20141206)
    /// </summary>
    public class CharCode
    {   ////////////////////////////////////////////////////////////////////////
        // <CharCode.cs> ReadJEnc 文字コード種類定義(Rev.20141206)
        ...
        /// <summary>50221 iso-2022-jp 日本語 (JIS-Allow 1 byte Kana) ※MS版＋JIS補助漢字独自対応</summary><remarks>JIS補助漢字はデコードのみ対応、エンコードは未対応</remarks>
        public static readonly Text JIS = new JisH("JIS50221", 50221);
        /// <summary>50222 iso-2022-jp 日本語 (JIS-Allow 1 byte Kana - SO/SI) ※MS版＋JIS補助漢字独自対応</summary><remarks>SO/SIによるカナシフトのみのファイルもCP50222とみなす。JIS補助漢字はデコードのみ対応、エンコードは未対応</remarks>
        public static readonly Text JIS50222 = new JisH("JIS50222", 50222);
        ...
        /// <summary>
        /// JIS補助漢字対応(補助漢字の部分はMS版CP20932でデコードする)
        /// </summary>
        class JisH : Text
        {
            internal JisH(string Name, int Encoding) : base(Name, Encoding) { }
            ...
            public override string GetString(byte[] Bytes, int Length)
            {
                try
                {
                    ...
                    {   //JIS補助漢字エスケープ以外の範囲を把握
                        ...
                        while (pos < Length)
                        {
                            if (Bytes[pos] == 0x1B && pos + 3 < Length &&
                                Bytes[pos + 1] == 0x24 &&
                                Bytes[pos + 2] == 0x28 &&
                                Bytes[pos + 3] == 0x44)
                        ...
                        if (start < pos)
                        {   //通常のCP5022Xでデコードする
                            ret.Append(GetEncoding().GetString(Bytes, start, pos - start));
                        }
                        //JIS補助漢字エスケープ部分の処理
                        if (pos < Length)
                        {   //JIS補助漢字エスケープシーケンス除去、補助漢字範囲特定
                            pos = pos + 4;
                            start = pos;
                            while (pos < Length && Bytes[pos] != 0x1B) { pos++; }
                            if (start < pos)
                            {
